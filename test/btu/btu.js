...
const BTUAbstraction = artifacts.require('BTU');
let HST;

contract('BTU', (accounts) => {
  beforeEach(async () => {
    HST = await BTUAbstraction.new(10000, 'Simon Bucks', 1, 'SBX', { from: accounts[0] });
  });
  ...
  it('creation: should succeed in creating over 2^256 - 1 (max) tokens', async () => {
    // 2^256 - 1
    const HST2 = await BTUAbstraction.new('115792089237316195423570985008687907853269984665640564039457584007913129639935', 'Simon Bucks', 1, 'SBX', { from: accounts[0] });
